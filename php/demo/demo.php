...
if ($format == "turtle") {
	$ser = ARC2::getTurtleSerializer ( array (
			'ns' => $ns 
	) );
	...
	$ser = ARC2::getRDFXMLSerializer ( array (
			'ns' => $ns 
	) );
	$output = $ser->getSerializedTriples ( $alltriples );
	...
	$ser = ARC2::getRDFJSONSerializer ( array (
			'ns' => $ns 
	) );
	$output = $ser->getSerializedTriples ( $alltriples );
	...
	$ser = ARC2::getNTriplesSerializer ( array (
			'ns' => $ns 
	) );
	$output = $ser->getSerializedTriples ( $alltriples );
...
<body>
	...
	<form>
		<div class="col_4">
			<p id="about">
				This web site show cases and combines available demo nif web
				services. Note that the list displayed here is not complete. More
				tools will be added soon. Read the <a
					href="http://svn.aksw.org/papers/2012/NIF/EKAW_DEMO/public_preprint.pdf"
		...
		<div class="col_4">
			<fieldset>
				...
				<input type="radio" id="radio_action_1" name="action" value="merge"
					checked="checked" /> <label for="radio_action_1">&nbsp;display and
					merge</label><br /> <input type="radio" id="radio_action_2"
					name="action" value="validate" disabled="disabled" /> <label
					for="radio_action_2">&nbsp;validate</label><br /> <input
					type="radio" id="radio_action_3" name="action" value="summarize"
					disabled="disabled" /> <label for="radio_action_3">&nbsp;summarize</label>
			</fieldset>
			<fieldset>
				<legend>URI Recipe</legend>
				<input type="radio" id="radio_uri_1" name="urirecipe" value="offset"
					checked="checked" /> <label for="radio_uri_1">&nbsp;offset</label><br />
				<input type="radio" id="radio_uri_2" name="urirecipe"
					value="context-hash" /> <label for="radio_uri_2">&nbsp;context-hash</label>
			</fieldset>
			<fieldset>
				<legend>Prefix</legend>
				<input id="text1" type="text" name="prefix"
	...
	<label for="text2"><p style="display: inline">Other NIF service:</p>&nbsp;</label><input
			...
			<div style="float: right">
				<button type="reset" class="medium red">
					<span class="icon medium white" data-icon="x"></span> reset
				</button>
				<button type="submit" class="medium green">
					<span class="icon medium white" data-icon="C"></span> submit
				</button>
			</div>
		</div>
